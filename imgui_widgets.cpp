...
// New Tab we use modified by 13yz
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if (hovered && it_filled->second >= 1.f)
        it_filled->second = ImClamp(it_filled->second - 2.2f * ImGui::GetIO().DeltaTime, 0.5f, 1.f);
    else
        it_filled->second = ImClamp(it_filled->second + (13.2f * ImGui::GetIO().DeltaTime * ((hovered) ? 1.f : -1.f)), 0.0f, 1.f);

    draw->AddRect(bb.Min, bb.Max, ImColor(14, 14, 16), 3, 15, 2);
    draw->AddRect(bb.Min, bb.Max, ImColor(38, 38, 42), 3, 15, 1);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, 25), window->DC.CursorPos + ImVec2(expected_w, 45));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(0, 27));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(14, 14, 16), 3, popup_open ? 3 : 15, 2);
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(38, 38, 42), 3, popup_open ? 3 : 15, 1);
    ...
    if (label_size.x > 0)
        RenderText(ImVec2(frame_bb.Min.x + 7, frame_bb.Min.y + style.FramePadding.y - 23), label);
    ...
    else
    {
        ...
        SetNextWindowSizeConstraints(ImVec2(expected_w, 0.0f), ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items)));
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // Draw frame
    draw->AddLine(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 4), ImVec2(frame_bb.Max.x, frame_bb.Min.y + 4), GetColorU32(ImGuiCol_Slider), 3);
    draw->AddLine(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 4), ImVec2(grab_bb.Min.x, frame_bb.Min.y + 4), GetColorU32(ImGuiCol_CheckMark), 3);
    draw->AddRect(frame_bb.Min, frame_bb.Max, ImColor(38, 38, 42), 3, 15, 2);
    draw->AddRect(frame_bb.Min, frame_bb.Max, ImColor(14, 14, 16), 3, 15, 1);

    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);

    RenderTextClipped(ImVec2(frame_bb.Max.x - 15, frame_bb.Min.y - 34), ImVec2(frame_bb.Max.x + 10, frame_bb.Max.y), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
    ...
    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Min.x + 7, total_bb.Min.y - 19), label);
...
bool ImGui::InputText(const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    ...
    return InputTextEx(label, NULL, buf, (int)buf_size, ImVec2(150, 20), flags, callback, user_data);
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImRect total_bb(ImVec2(frame_bb.Min.x, frame_bb.Min.y - 15), frame_bb.Max);
    ...
    {
        RenderNavHighlight(frame_bb, id);
        RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBg), true, 4);
    }
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    const ImRect coloredit_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(250, 20));
    const float w_inputs = w_full - w_button;
    
    const char* label_display_end = FindRenderedTextEnd(label);
    g.NextItemData.ClearFlags();

    ItemSize(coloredit_bb);
    BeginGroup();
    PushID(label);
    draw->AddRectFilled(coloredit_bb.Min, coloredit_bb.Max, ImColor(40, 40, 40), 5);
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        RenderText(ImVec2(coloredit_bb.Min.x + 5, coloredit_bb.Min.y + 3), label);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f)
    {
        ...
        window->DrawList->AddRectFilled(bb_inner.Min, ImVec2(mid_x, bb_inner.Max.y), GetColorU32(col_rgb_without_alpha), 5, ImDrawCornerFlags_TopLeft | ImDrawCornerFlags_BotLeft);
    }
    else
    {
        ...
        if (col_source.w < 1.0f)
            ...
            window->DrawList->AddRectFilled(bb_inner.Min, bb_inner.Max, GetColorU32(col_source), 5, ImDrawCornerFlags_All);
    }
    RenderNavHighlight(bb, id);
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0)
    {
...
bool ImGui::sub(const char* label, bool selected)
{
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = ImGui::CalcItemSize(ImVec2(125, 20), label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
    ...
    if (g.LastActiveId == g.CurrentWindow->GetID(label))
    {
        ...
        t = selected ? (t_anim) : (1.f - t_anim);
    }
    ImU32 text;

    text = GetColorU32(ImLerp(GetStyleColorVec4(ImGuiCol_Text), ImVec4(254 / 255.f, 151 / 255.f, 98 / 255.f, 255 / 255.f), t));
    draw->AddRectFilled(bb.Min, bb.Max, GetColorU32(ImGuiCol_WindowBg), 10);
    ImGui::PushStyleColor(ImGuiCol_Text, text);
    RenderText(ImVec2(bb.Min.x + size.x / 2 - label_size.x / 2, bb.Min.y + size.y / 2 - label_size.y / 2), label);
    ImGui::PopStyleColor();
...
bool ImGui::style(const char* label, bool selected)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::lang(const char* label, bool selected)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    {
        ImGui::PushFont(iconfont);
        ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(174 / 255.f, 129 / 255.f, 19 / 255.f, 1.f));
        RenderText(bb.Min, label);
        ImGui::PopStyleColor();
        ImGui::PopFont();
    }
